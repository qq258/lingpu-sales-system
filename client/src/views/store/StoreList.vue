  ...
  <div class="pbm-root">
    <header class="pbm-header">
      <div class="pbm-header-left">
        <h1 class="pbm-title">门店管理</h1>
        <span class="pbm-subtitle">Store Management</span>
      </div>
      ...
      <div class="pbm-main">
        <div class="pbm-main-head">
          <div class="pbm-search-group">
            ...
            <select v-model="searchStatus" class="pbm-select" @change="onSearch">
              <option :value="undefined">全部状态</option>
              <option :value="1">已启用</option>
              <option :value="0">已禁用</option>
            </select>
          </div>
          <span class="pbm-section-count">{{ filteredStores.length }} 个门店</span>
        </div>

        <div class="pbm-table-wrapper">
          <el-table :data="filteredStores" stripe v-loading="loading" size="small" element-loading-background="rgba(245,240,235,0.8)">
            ...
            <el-table-column label="状态" width="100" align="center">
              <template #default="{ row }">
                <span v-if="!userStore.isSuperAdmin" class="pbm-status-tag" :class="row.status === 1 ? 'pbm-status-tag--on' : 'pbm-status-tag--off'">
                  {{ row.status === 1 ? '启用' : '禁用' }}
                </span>
                <button
                  v-else
                  class="pbm-status-toggle"
                  :class="{ 'is-on': row.status === 1 }"
                  @click="handleStatusChange(row)"
                  :title="row.status === 1 ? '点击禁用' : '点击启用'"
                >
                  <span class="pbm-status-toggle-track">
                    <span class="pbm-status-toggle-thumb" />
                  </span>
                </button>
              </template>
            </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑门店' : '新增门店'" width="520px" :close-on-click-modal="false" class="pbm-dialog">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px" size="default">
        <el-form-item label="门店名称" prop="name">
          <el-input v-model="form.name" placeholder="请输入门店名称" maxlength="100" show-word-limit />
        </el-form-item>
        <el-form-item label="门店编码" prop="code">
          <el-input v-model="form.code" placeholder="如：A、B、C" maxlength="20" show-word-limit />
        </el-form-item>
...
import type { StoreData } from '@/api/store'

const userStore = useUserStore()

const loading = ref(false)
const submitLoading = ref(false)
const stores = ref<(StoreData & { _statusLoading?: boolean })[]>([])
...
const filteredStores = computed(() => {
  ...
  if (searchStatus.value !== undefined && searchStatus.value !== null) {
    list = list.filter(s => s.status === searchStatus.value)
  }
  return list
})
...
async function loadStores() {
  loading.value = true
  try {
    stores.value = (await getStores()).map(s => ({ ...s, _statusLoading: false }))
  } catch {
    stores.value = []
  } finally {
    loading.value = false
...
function openDialog(row?: StoreData) {
  if (row) {
    isEdit.value = true
    editId.value = row.id!
    form.value = {
    ...
    isEdit.value = false
    editId.value = null
    form.value = { name: '', code: '', address: '', phone: '', status: 1 }
  }
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value && editId.value) {
      await updateStore(editId.value, form.value)
...
async function handleStatusChange(row: any) {
  ...
  try {
    await updateStore(row.id!, { status: newStatus })
    ElMessage.success(newStatus ? '门店已启用' : '门店已禁用')
  } catch {
    row.status = prev
  } finally {
...
async function handleDelete(row: StoreData) {
  try {
    await ElMessageBox.confirm(`确定删除门店「${row.name}」吗？该操作不可恢复。`, '确认删除', {
      type: 'warning',
...
.pbm-select:focus { border-color: var(--pbm-accent); box-shadow: 0 0 0 2px var(--pbm-accent-glow); }
.pbm-section-count {
  font-size: 12px;
  color: var(--pbm-text-dim);
...
.pbm-status-tag--on { background: rgba(46,125,50,0.1); color: #2e7d32; }
.pbm-status-tag--off { background: rgba(220,53,69,0.1); color: var(--pbm-red); }
...
.pbm-status-toggle.is-on .pbm-status-toggle-track { background: #2e7d32; }
.pbm-status-toggle-thumb {
  display: block;
  width: 16px;
  height: 16px;
  border-radius: 50%;
...
.pbm-status-toggle.is-on .pbm-status-toggle-thumb { transform: translateX(16px); }
