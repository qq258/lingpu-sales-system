  ...
  <div class="pbm-root">
    <header class="pbm-header">
      <div class="pbm-header-left">
        <h1 class="pbm-title">型号管理</h1>
        <span class="pbm-subtitle">Model Management</span>
      </div>
        ...
        <div class="pbm-main-head">
          <div class="pbm-search-group">
            <div class="pbm-search-input-wrapper">
              <el-select v-model="filterBrandId" placeholder="选择品牌" clearable filterable size="small" @change="loadModels">
                <el-option v-for="b in brands" :key="b.id" :label="b.name" :value="b.id" />
              </el-select>
            </div>
          </div>
          <span class="pbm-section-count">{{ models.length }} 个型号</span>
        </div>

        <div class="pbm-table-wrapper">
          <el-table :data="models" stripe v-loading="loading" size="small" element-loading-background="rgba(245,240,235,0.8)">
            <el-table-column type="index" label="#" width="48" align="center" />
            <el-table-column prop="brandName" label="所属品牌" min-width="120">
              <template #default="{ row }">
                <span class="pbm-cell-name">{{ row.brandName }}</span>
              </template>
            </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑型号' : '新增型号'" width="520px" :close-on-click-modal="false" class="pbm-dialog">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px" size="default">
        <el-form-item label="所属品牌" prop="brandId">
          <el-select v-model="form.brandId" placeholder="选择品牌" filterable style="width: 100%;">
            <el-option v-for="b in brands" :key="b.id" :label="b.name" :value="b.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="型号名称" prop="name">
          <el-input v-model="form.name" placeholder="请输入型号名称" maxlength="100" show-word-limit />
        </el-form-item>
...
import { getBrands, getModels, createModel, updateModel, deleteModel } from '@/api/product'
...
const filterBrandId = ref<number | undefined>()

const form = ref<ModelData>({ brandId: 0, name: '', description: '' })
...
async function loadModels() {
  loading.value = true
  try {
    models.value = await getModels(filterBrandId.value)
  } catch {
    models.value = []
  } finally {
    loading.value = false
...
function openDialog(row?: ModelData) {
  if (row) {
    isEdit.value = true
    editId.value = row.id!
    form.value = { ...row }
    ...
    isEdit.value = false
    editId.value = null
    form.value = { brandId: 0, name: '', description: '' }
  }
...
async function handleDelete(row: ModelData) {
  try {
    await ElMessageBox.confirm(`确定删除型号「${row.name}」吗？`, '确认删除', {
      type: 'warning',
...
.pbm-search-input-wrapper :deep(.el-select) { width: 160px; }
