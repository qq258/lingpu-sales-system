  ...
  <div class="pbm-root">
    <header class="pbm-header">
      <div class="pbm-header-left">
        <h1 class="pbm-title">品牌管理</h1>
        <span class="pbm-subtitle">Brand Management</span>
      </div>
    ...
    <div class="pbm-body">
      <div class="pbm-main">
        <div class="pbm-main-head">
          <div class="pbm-search-group"></div>
          <span class="pbm-section-count">{{ brands.length }} 个品牌</span>
        </div>

        <div class="pbm-table-wrapper">
          <el-table :data="brands" stripe v-loading="loading" size="small" element-loading-background="rgba(245,240,235,0.8)">
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑品牌' : '新增品牌'" width="520px" :close-on-click-modal="false" class="pbm-dialog">
...
const form = ref<BrandData>({ name: '', description: '' })
...
function openDialog(row?: BrandData) {
  if (row) {
    isEdit.value = true
    editId.value = row.id!
    form.value = { ...row }
...
async function handleDelete(row: BrandData) {
  try {
    await ElMessageBox.confirm(`确定删除品牌「${row.name}」吗？`, '确认删除', {
      type: 'warning',
