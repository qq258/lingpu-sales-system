  ...
  <div class="pbm-root">
    <header class="pbm-header">
      <div class="pbm-header-left">
        <h1 class="pbm-title">SKU 管理</h1>
        <span class="pbm-subtitle">SKU Management</span>
      </div>
          ...
          <div class="pbm-search-group">
            <div class="pbm-search-input-wrapper">
              ...
              <input
                ...
                placeholder="搜索SKU名称 / 条码"
                @input="onSearchInput"
              />
            </div>
            <div class="pbm-search-input-wrapper">
              <el-select v-model="searchBrandId" placeholder="选择品牌" clearable filterable size="small" @change="onBrandFilterChange">
                <el-option v-for="b in brands" :key="b.id" :label="b.name" :value="b.id" />
              </el-select>
            </div>
            <div class="pbm-search-input-wrapper">
              <el-select v-model="searchModelId" placeholder="选择型号" clearable filterable size="small">
                <el-option v-for="m in filteredModels" :key="m.id" :label="m.name" :value="m.id" />
              </el-select>
            </div>
            <button class="pbm-btn-accent pbm-btn-accent--sm" @click="loadSkus">查询</button>
          </div>
          <span class="pbm-section-count">{{ total }} 个SKU</span>
        </div>

        <div class="pbm-table-wrapper">
          <el-table :data="skuList" stripe v-loading="loading" size="small" element-loading-background="rgba(245,240,235,0.8)">
            ...
            <el-table-column prop="price" label="售价" width="100" align="right">
              <template #default="{ row }">
                <span class="pbm-cell-price">¥{{ row.price?.toFixed(2) }}</span>
              </template>
            </el-table-column>
            <el-table-column prop="costPrice" label="成本价" width="100" align="right">
              <template #default="{ row }">
                <span class="pbm-cell-price pbm-cell-price--muted">¥{{ row.costPrice?.toFixed(2) || '-' }}</span>
              </template>
            </el-table-column>
            <el-table-column prop="barcode" label="条码" width="140" />
            <el-table-column prop="stock" label="库存" width="80" align="center">
              <template #default="{ row }">
                <span :class="['pbm-stock-tag', (row.stock ?? 0) <= 0 ? 'pbm-stock-tag--danger' : (row.stock ?? 0) <= 5 ? 'pbm-stock-tag--warn' : 'pbm-stock-tag--ok']">
                  {{ row.stock ?? 0 }}
                </span>
              </template>
            </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑SKU' : '新增SKU'" width="600px" :close-on-click-modal="false" class="pbm-dialog">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px" size="default">
        <el-form-item label="品牌" prop="brandId">
          <el-select v-model="form.brandId" placeholder="选择品牌" filterable @change="onFormBrandChange">
            <el-option v-for="b in brands" :key="b.id" :label="b.name" :value="b.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="型号" prop="modelId">
          <el-select v-model="form.modelId" placeholder="选择型号" filterable>
            <el-option v-for="m in formModels" :key="m.id" :label="m.name" :value="m.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="颜色" prop="color">
          <el-input v-model="form.color" placeholder="如：黑色、白色" />
        </el-form-item>
        <el-form-item label="存储" prop="storage">
          <el-input v-model="form.storage" placeholder="如：128G、256G" />
        </el-form-item>
        ...
        <el-form-item label="厂家条码" prop="barcode">
          <el-input v-model="form.barcode" placeholder="扫码枪扫描条码" />
        </el-form-item>
      </el-form>
      <template #footer>
...
import { getBrands, getModels, getSkus, createSku, updateSku, deleteSku } from '@/api/product'
...
const filteredModels = computed(() => {
  if (!searchBrandId.value) return allModels.value
  return allModels.value.filter(m => m.brandId === searchBrandId.value)
})

const formModels = ref<ModelData[]>([])

const form = ref<SkuData>({
  brandId: 0,
  modelId: 0,
  color: '',
  storage: '',
...
const rules: FormRules = {
  ...
  storage: [{ required: true, message: '请输入存储规格', trigger: 'blur' }],
...
async function loadSkus() {
  ...
  try {
    ...
    if (searchBrandId.value) params.brandId = searchBrandId.value
    if (searchModelId.value) params.modelId = searchModelId.value
    const result = await getSkus(params)
    skuList.value = result.list
...
function onFormBrandChange(brandId: number) {
  form.value.modelId = 0
  formModels.value = allModels.value.filter(m => m.brandId === brandId)
}

function openDialog(row?: SkuData) {
  if (row) {
    isEdit.value = true
    editId.value = row.id!
    form.value = { ...row }
    formModels.value = allModels.value.filter(m => m.brandId === row.brandId)
  } else {
    isEdit.value = false
    editId.value = null
    form.value = { brandId: 0, modelId: 0, color: '', storage: '', price: 0, costPrice: 0, barcode: '' }
    formModels.value = []
  }
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value && editId.value) {
      await updateSku(editId.value, form.value)
...
async function handleDelete(row: SkuData) {
  try {
    await ElMessageBox.confirm(`确定删除该SKU吗？`, '确认删除', {
      type: 'warning',
...
.pbm-search-input-wrapper:has(.el-select) { padding: 2px 8px; }
...
.pbm-cell-price--muted { color: var(--pbm-text-dim); }
...
.pbm-stock-tag--ok { background: rgba(34,197,94,0.12); color: #16a34a; }
.pbm-stock-tag--warn { background: rgba(234,179,8,0.12); color: #ca8a04; }
.pbm-stock-tag--danger { background: var(--pbm-red-dim); color: var(--pbm-red); }
...
:deep(.pbm-dialog .el-input-number .el-input__wrapper) { padding-right: 0; }
:deep(.pbm-dialog .el-input-number .el-input-number__decrease),
:deep(.pbm-dialog .el-input-number .el-input-number__increase) { background: #ede7e0; color: var(--pbm-text-dim); }
:deep(.pbm-dialog .el-input-number .el-input-number__decrease:hover),
:deep(.pbm-dialog .el-input-number .el-input-number__increase:hover) { color: var(--pbm-text); }
